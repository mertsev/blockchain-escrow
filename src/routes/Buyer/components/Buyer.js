...
// Use Mist/MetaMask's provider
    ...
    function initContract (contract) {
      var _address = prompt('Введите адрес контракта данный продавцом')
      while(_address === "" || _address === null ){
        _address = prompt('Ошибка.Введите адрес контракта данный продавцом')
        check = true
      }
      check = true
      var contractAbi = web3.eth.contract(abi)
      var contractInstance = contractAbi.at(_address)
  ...
  indicateSuccess()
    ...
    function listenForClicks (contractInstance) {
      ...
      confirm.addEventListener('click', function () {
        if(check){
        var __value = prompt('Введите сумму указанную продавцом')
        contractInstance.confirmPurchase({ from:provider, value:2 * __value },function(error, result){
    if(!error)
        console.log(result)
    else
        console.error(error);
        ...
        alert('Вы не присоеденили контракт')
      ...
      recieve.addEventListener('click', function () {
         if(check){
        contractInstance.confirmRecieved({from:provider}, function(error, result){
    if(!error)
        console.log(result)
    else
        console.error(error);
    ...
    return (
      <div style={{ margin: '0 auto' }} >
        <div className='form-group'>
          ...
          <button className='btn' id="connect"> Присоеденить контракт</button>
 
          <button className='btn' id="confirm" > Подтвердить покупку</button>
          <button className='btn' id="recieve" > Подтвердить получение</button>
